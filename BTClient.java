...
public class BTClient extends JFrame implements ActionListener {
   ...
   public BTClient(){
      ...
      enter();
	  bgplayer();
	  if(gamestart=false) clipmain.start();
	  else if(gamestart==true) clipmain.close();
   }
    void init(){
      cp = getContentPane();
	  cp.setLayout(null);
	...
	void bgplayer(){
		try{
		   File file = new File("sound/bgmusic2.wav");
		   AudioInputStream audioInputStream = AudioSystem.getAudioInputStream(file);
		   clipmain = AudioSystem.getClip();
		   clipmain.open(audioInputStream);
    ...
    void enter(){
      ...
      int charac = Start.charac;
       ...
       try{
         ...
         Listening listen = new Listening(soc);
         speak.start();
         listen.start();
